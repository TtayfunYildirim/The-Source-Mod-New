...
public class FlamethrowerUsedProcedure {
	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		if ((entity.getCapability(TheSourceModVariables.PLAYER_VARIABLES_CAPABILITY, null)
				.orElse(new TheSourceModVariables.PlayerVariables())).flowlevel >= 10) {
			{
				...
				if (!projectileLevel.isRemote()) {
					ProjectileEntity _entityToSpawn = new Object() {
						public ProjectileEntity getArrow(World world, Entity shooter, float damage, int knockback) {
							AbstractArrowEntity entityToSpawn = new FlamethrowerProjectileItem.ArrowCustomEntity(FlamethrowerProjectileItem.arrow,
									world);
